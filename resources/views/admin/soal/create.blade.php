...
imageButton.addEventListener('click', () => {
    fileInput.click(); // Trigger input file saat tombol ditekan
});

// Tambahkan event listener untuk menangkap file yang diunggah
fileInput.addEventListener('change', (event) => {
    const file = event.target.files[0];
    if (file) {
        alert(`Gambar "${file.name}" berhasil dipilih.`);
        // Anda bisa menambahkan logika di sini untuk mengunggah gambar ke server
    }
});
