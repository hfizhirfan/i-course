        ...
        <div class="modal-dialog modal-dialog-centered">
          <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="upload">
                    <!-- Add File Button -->
                    <div class="upload-section mb-4 text-center" id="file-upload" onclick="document.getElementById('fileInput').click();">
                        <i class="bi bi-upload fs-2"></i>
                        <p id="file-name" class="mt-2">Drag and drop file</p>
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    // Awalnya tertutup
    if (notifPopup) notifPopup.classList.add("d-none");
    ...
    if (profileImage && profilePopup) {
        profileImage.addEventListener("click", function (e) {
            e.stopPropagation();
            profilePopup.style.display = (profilePopup.style.display === "none" || profilePopup.style.display === "")
                ? "block"
                : "none";
    ...
    if (importModal) {
        importModal.addEventListener('show.bs.modal', function () {
            ...
            if (fileName) fileName.textContent = 'Drag and drop file';
        });
    }

    if (fileInput && fileName) {
        fileInput.addEventListener('change', function () {
            fileName.textContent = this.files[0]?.name || 'Drag and drop file';
        });
    }
 ...
 // Tampilkan nama file di tengah upload section
 document.getElementById('fileInput').addEventListener('change', function () {
    const fileName = this.files[0]?.name || "Drag and drop file";
    document.getElementById('file-name').textContent = fileName;
  });

  // Tutup modal saat tombol Import ditekan
  document.querySelector('.btn-import').addEventListener('click', function () {
    const importModal = bootstrap.Modal.getInstance(document.getElementById('importSoalModal'));
    importModal.hide();
});
